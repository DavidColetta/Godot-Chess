...
func get_lobby_members() -> void:
	...
	$Players/PlayerCount.text = "Players("+str(num_of_members)+")"
...
func add_player_list(member_steam_id, member_steam_name) -> void:
	lobby_members.append({"steam_id":member_steam_id, "steam_name":member_steam_name})
	
	$Players/PlayerList.clear()
	
	for member in lobby_members:
		$Players/PlayerList.add_text(member["steam_name"]+'\n')

func display_chat_message(message: String) -> void:
	$Chat/RichTextLabel.add_text("\n" + message)
...
func _on_lobby_created(connect: int, this_lobby_id: int) -> void:
	if connect == 1:
		...
		print("Created a lobby: %s" % Globals.lobby_id)

		# Set this lobby as joinable, just in case, though this should be done by default
		Steam.setLobbyJoinable(Globals.lobby_id, true)

		# Set some lobby data
		Steam.setLobbyData(Globals.lobby_id, "name", $Create/LobbyName.text)
		Steam.setLobbyData(Globals.lobby_id, "mode", "GodotSteam test")

		# Allow P2P connections to fallback to being relayed through Steam if needed
		var set_relay: bool = Steam.allowP2PPacketRelay(true)
		print("Allowing Steam to be relay backup: %s" % set_relay)
		display_chat_message("Created a lobby "+$Create/LobbyName.text)
		$Chat/Name.text = $Create/LobbyName.text

#when lobby metadata changes
func _on_lobby_data_update(success, lobbyID, memberID, key):
	pass
	
#whenever something changes in the lobby
func _on_lobby_chat_update(this_lobby_id: int, change_id: int, making_change_id: int, chat_state: int) -> void:
	...
	if chat_state == Steam.CHAT_MEMBER_STATE_CHANGE_ENTERED:
		display_chat_message("%s has joined the lobby." % changer_name)

	# Else if a player has left the lobby
	elif chat_state == Steam.CHAT_MEMBER_STATE_CHANGE_LEFT:
		display_chat_message("%s has left the lobby." % changer_name)

	# Else if a player has been kicked
	elif chat_state == Steam.CHAT_MEMBER_STATE_CHANGE_KICKED:
		display_chat_message("%s has been kicked from the lobby." % changer_name)

	# Else if a player has been banned
	elif chat_state == Steam.CHAT_MEMBER_STATE_CHANGE_BANNED:
		display_chat_message("%s has been banned from the lobby." % changer_name)

	# Else there was some unknown change
	else:
		display_chat_message(str(chat_state) + ": %s did... something." % changer_name)
...
func _on_lobby_message(result, user, message, type):
	...
	display_chat_message(str(sender) + ": "+message)

func _fetch_lobby_list() -> void:
...
func _on_lobby_match_list(these_lobbies: Array) -> void:
	for this_lobby in these_lobbies:
		...
		$LobbyList/Scroll/List.add_child(lobby_button)
...
func _on_send_chat_pressed() -> void:
	...
	var this_message: String = $SendMessage/MessageInput.get_text()

	# If there is even a message
	if this_message.length() > 0:
		# Pass the message to Steam
		var was_sent: bool = Steam.sendLobbyChatMsg(Globals.lobby_id, this_message)
	...
	$SendMessage/MessageInput.clear()
